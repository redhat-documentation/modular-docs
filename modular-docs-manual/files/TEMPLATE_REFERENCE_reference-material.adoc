...
[id="ref-my-reference-a_{context}"]
////
...
[role="_abstract"]
Write a short introductory paragraph that provides an overview of the module. The text that immediately follows the `[role="_abstract"]` tag up to a line break is used for search metadata. A reference module provides data that users might want to look up, but do not need to remember.
